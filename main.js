...
(function(){
  ...
  function initializeNetflixEnhancements(){
    ...
    document.querySelectorAll('section, .project-section, .cert-section').forEach(section => {
      section.classList.add('fade-in-section');
      fadeObserver.observe(section);
    });
    
    // Enhanced card loading simulation
    ...
    document.querySelectorAll('h1, h2, .hero-title').forEach(heading => {
  ...
  // Add animation class for smooth shrinking
  this.modal.classList.add('animating-out');
  
  // Wait for animation to complete before hiding
  setTimeout(() => {
    this.modal.classList.remove('active', 'animating-out');
    this.modal.setAttribute('aria-hidden','true');
    document.body.style.overflow='';
    if(this.lastFocus)this.lastFocus.focus({preventScroll:true});
    if(location.hash.startsWith('#project-')||location.hash.startsWith('#cert-'))history.replaceState(null,'',location.pathname+location.search);
    this.current=null;
  }, 400);
}navigate(dir){if(!this.current)return;const idx=this.projectOrder.indexOf(this.current);const nextIdx=idx+dir;if(nextIdx<0||nextIdx>=this.projectOrder.length)return;const nextId=this.projectOrder[nextIdx];const card=document.querySelector(`.netflix-card[data-project="${nextId}"]`);this.showProjectDetail(nextId,card);}updateProgress(){const prog=this.modal.querySelector('.project-progress');if(!prog||!this.current)return;const idx=this.projectOrder.indexOf(this.current);prog.textContent=`${idx+1} / ${this.projectOrder.length}`;}updateNavButtons(){const prev=this.modal.querySelector('.project-nav-btn.prev');const next=this.modal.querySelector('.project-nav-btn.next');const idx=this.projectOrder.indexOf(this.current);prev.disabled=idx<=0;next.disabled=idx>=this.projectOrder.length-1;[prev,next].forEach(b=>b.style.opacity=b.disabled?'.35':'1');}copyLink(){if(!this.current)return;const url=location.origin+location.pathname+`#project-${this.current}`;navigator.clipboard.writeText(url).then(()=>{showToast('Link copied');this.announce('Link copied');}).catch(()=>showToast('Copy failed'));}ensureQrButton(){if(this.modal.querySelector('.project-qr-btn'))return;const bar=this.modal.querySelector('.project-nav-bar');const btn=document.createElement('button');btn.type='button';btn.className='project-qr-btn';btn.textContent='QR';btn.setAttribute('aria-label','Show QR code');btn.style.cssText='font-size:.7rem;padding:.35rem .55rem;border-radius:6px;border:1px solid rgba(255,255,255,.15);background:rgba(255,255,255,.08);';btn.addEventListener('click',()=>this.showQr());bar.appendChild(btn);}showQr(){if(!this.current)return;const share=encodeURIComponent(location.origin+location.pathname+`#project-${this.current}`);const qr=`https://api.qrserver.com/v1/create-qr-code/?size=220x220&data=${share}`;let o=document.querySelector('.qr-overlay');if(!o){o=document.createElement('div');o.className='qr-overlay';o.style.cssText='position:fixed;inset:0;background:rgba(0,0,0,.65);display:flex;align-items:center;justify-content:center;z-index:2000001;';o.innerHTML=`<div class=\"qr-box\" style=\"background:#111;padding:1.2rem 1.4rem 1.4rem;border-radius:14px;position:relative;box-shadow:0 8px 32px -8px rgba(0,0,0,.7);text-align:center;max-width:280px;\"><button class=\"qr-close\" aria-label=\"Close\" style=\"position:absolute;top:6px;right:6px;background:rgba(255,255,255,.12);border:none;color:#fff;width:30px;height:30px;border-radius:50%;cursor:pointer;\">✕</button><h3 style=\"margin:0 0 .75rem;font-size:.9rem;letter-spacing:.05em;font-weight:600;\">Share Project</h3><img class=\"qr-img\" src=\"${qr}\" alt=\"QR code\" style=\"width:220px;height:220px;border-radius:8px;background:#000;object-fit:contain;\"/><p style=\"margin:.7rem 0 0;font-size:.65rem;line-height:1.2;color:#aaa;\">Scan on your phone<br><span style=\"color:#fff;\">${this.current}</span></p></div>`;document.body.appendChild(o);o.addEventListener('click',e=>{if(e.target===o)o.remove();});o.querySelector('.qr-close').addEventListener('click',()=>o.remove());}else{o.querySelector('.qr-img').src=qr;const span=o.querySelector('span');span&&(span.textContent=this.current);}showToast('QR code ready');}announce(msg){if(this.live){this.live.textContent='';requestAnimationFrame(()=>{this.live.textContent=msg;});}}getProjectData(id,card){if(!this._data)this._data=this.staticData();if(this._data[id])return this._data[id];if(card){const title=card.querySelector('.netflix-card-title')?.textContent?.trim()||id;const desc=card.querySelector('.netflix-card-description')?.textContent?.trim()||'';return{id,title,subtitle:'Project',image:card.querySelector('img')?.src||'',description:desc,longDescription:desc,metrics:[],technologies:[],features:[],architecture:{title:'Architecture',description:'',components:[]},github:null,demo:null};}return null;}staticData(){return{'financial-risk':{title:'📈 GPU-Accelerated Financial Risk Analysis',subtitle:'CUDA Monte Carlo / VaR',image:'https://images.unsplash.com/photo-1611974789855-9c2a0a7236a3?auto=format&fit=crop&w=1200&q=80',description:'GPU-accelerated risk simulation.',longDescription:'Accelerated financial computation using CUDA and GPU parallelism for large-scale data.',metrics:[{label:'Speedup',value:'1.57×',icon:'🚀'}],technologies:[{name:'CUDA'},{name:'PyCUDA'},{name:'NumPy'}],features:['Monte Carlo engine','VaR computation','Performance benchmarking'],architecture:{title:'System Architecture',description:'GPU kernel + analysis pipeline',components:['CUDA Kernels','Simulation Runner','Result Analyzer']},github:'https://github.com/saiprudvi0102/Finance-Risk-Analysis-GPU',demo:null},'mobilellm':{title:'📱 Edge-Deployed Mobile LLM Pipeline',subtitle:'Quantization / On-Device AI',image:'https://images.unsplash.com/photo-1510557880182-3d4d3cba35a5?auto=format&fit=crop&w=1200&q=80',description:'Deploying compressed transformer models to mobile hardware.',longDescription:'Fine-tuned a lightweight large language model for mobile-friendly zero-shot tasks.',metrics:[{label:'Latency',value:'<120ms',icon:'⚡'},{label:'RAM Footprint',value:'-42%',icon:'🧠'}],technologies:[{name:'PyTorch'},{name:'TorchScript'},{name:'ONNX'},{name:'CoreML'}],features:['Quantization-aware training','Pruning + distillation','Operator fusion','Adaptive batching'],architecture:{title:'Pipeline',description:'Conversion + optimization + runtime interface',components:['Training Artifacts','Exporter','Quantizer','Runtime Adapter','Mobile Inference Engine']},github:'https://github.com/saiprudvi0102/MobileLLM125M',demo:null},'genai-clinical':{title:'🏥 Generative AI for Clinical Decision Support',subtitle:'Healthcare LLM / Medical NLP',image:'https://images.unsplash.com/photo-1576091160399-112ba8d25d1f?auto=format&fit=crop&w=1200&q=80',description:'AI-powered clinical decision support system.',longDescription:'Advanced generative AI system designed to assist healthcare professionals with clinical decision-making through intelligent data analysis and evidence-based recommendations.',metrics:[{label:'Accuracy',value:'94.2%',icon:'🎯'},{label:'Response Time',value:'<3s',icon:'⚡'}],technologies:[{name:'Transformers'},{name:'BERT'},{name:'BioBERT'},{name:'FastAPI'},{name:'React'}],features:['Medical entity recognition','Clinical note analysis','Drug interaction checking','Evidence-based recommendations'],architecture:{title:'System Architecture',description:'Multi-modal AI pipeline for clinical data',components:['NLP Preprocessing','Medical Entity Extraction','Knowledge Graph Integration','Decision Support Engine','Web Interface']},github:'https://github.com/saiprudvi0102/GenAI-Clinical',demo:null},'telecom-microservices':{title:'📡 Telecom Microservices Platform',subtitle:'5G Network Management / Cloud-Native',image:'https://images.unsplash.com/photo-1558618666-fcd25c85cd64?auto=format&fit=crop&w=1200&q=80',description:'Scalable microservices architecture for telecom operations.',longDescription:'Enterprise-grade microservices platform designed for telecom network management, featuring 5G network slicing, real-time monitoring, and automated scaling capabilities.',metrics:[{label:'Throughput',value:'1M req/s',icon:'🚀'},{label:'Uptime',value:'99.9%',icon:'⏰'},{label:'Latency',value:'<50ms',icon:'⚡'}],technologies:[{name:'Spring Boot'},{name:'Kubernetes'},{name:'Docker'},{name:'Apache Kafka'},{name:'Redis'},{name:'PostgreSQL'},{name:'Istio'}],features:['5G network slicing','Real-time analytics','Auto-scaling','Circuit breaker pattern','Distributed tracing','API gateway'],architecture:{title:'Microservices Architecture',description:'Cloud-native telecom service mesh',components:['API Gateway','User Management Service','Network Monitoring Service','Billing Service','Analytics Engine','Message Queue','Service Mesh']},github:'https://github.com/saiprudvi0102/Telecom-Microservices',demo:null},'dormunity':{title:'🏠 Dormunity - Smart Campus Community',subtitle:'React Native / Firebase / Social Platform',image:'https://images.unsplash.com/photo-1523240795612-9a054b0db644?auto=format&fit=crop&w=1200&q=80',description:'Social platform connecting campus residents.',longDescription:'Mobile application designed to foster community among university dormitory residents through event organization, resource sharing, and social networking features.',metrics:[{label:'Active Users',value:'2.5K+',icon:'👥'},{label:'App Rating',value:'4.8★',icon:'⭐'},{label:'Events Created',value:'500+',icon:'📅'}],technologies:[{name:'React Native'},{name:'Firebase'},{name:'Node.js'},{name:'Express.js'},{name:'MongoDB'}],features:['Event organization','Resource marketplace','Study group matching','Roommate finder','Campus notifications','Social feed'],architecture:{title:'Mobile Architecture',description:'Cross-platform social networking app',components:['React Native Frontend','Firebase Authentication','Real-time Database','Push Notifications','Image Storage','API Server']},github:'https://github.com/saiprudvi0102/Dormunity',demo:null},'stylestitch':{title:'✂️ StyleStitch - AI Fashion Platform',subtitle:'Computer Vision / E-commerce / ML',image:'https://images.unsplash.com/photo-1441986300917-64674bd600d8?auto=format&fit=crop&w=1200&q=80',description:'AI-powered fashion recommendation and virtual styling.',longDescription:'Intelligent fashion platform leveraging computer vision and machine learning to provide personalized styling recommendations and virtual try-on experiences.',metrics:[{label:'Accuracy',value:'91%',icon:'🎯'},{label:'User Engagement',value:'+65%',icon:'📈'},{label:'Conversion Rate',value:'+23%',icon:'💰'}],technologies:[{name:'TensorFlow'},{name:'OpenCV'},{name:'React'},{name:'Python'},{name:'FastAPI'},{name:'PostgreSQL'}],features:['Visual similarity search','Virtual try-on','Style recommendation','Outfit generation','Trend analysis','Size prediction'],architecture:{title:'AI Pipeline',description:'Computer vision + recommendation engine',components:['Image Processing','Feature Extraction','Similarity Engine','Recommendation Algorithm','Virtual Try-on','Web Interface']},github:'https://github.com/saiprudvi0102/StyleStitch',demo:null},'mobile-architecture':{title:'📱 Mobile App Architecture Framework',subtitle:'React Native / Native Modules / Performance',image:'https://images.unsplash.com/photo-1512941937669-90a1b58e7e9c?auto=format&fit=crop&w=1200&q=80',description:'Scalable mobile app architecture and performance optimization.',longDescription:'Comprehensive mobile application architecture framework focusing on performance optimization, native module integration, and scalable development patterns.',metrics:[{label:'Performance',value:'+40%',icon:'⚡'},{label:'Bundle Size',value:'-30%',icon:'📦'},{label:'Load Time',value:'-50%',icon:'⏱️'}],technologies:[{name:'React Native'},{name:'TypeScript'},{name:'Native Modules'},{name:'Redux'},{name:'Flipper'}],features:['Code splitting','Lazy loading','Memory optimization','Native bridge optimization','Performance monitoring','Automated testing'],architecture:{title:'Architecture Patterns',description:'High-performance mobile app structure',components:['Navigation Layer','State Management','API Layer','Native Modules','Performance Monitor','Testing Framework']},github:'https://github.com/saiprudvi0102/Mobile-Architecture',demo:null},'pcnn-shadow':{title:'🌫️ Parallel CNN Shadow Detection',subtitle:'Image Segmentation CUDA',image:'https://images.unsplash.com/photo-1501785888041-af3ef285b470?auto=format&fit=crop&w=1200&q=80',description:'GPU-accelerated shadow segmentation.',longDescription:'Image shadow removal using biologically inspired neural networks and OpenCV.',metrics:[{label:'Throughput',value:'240 FPS',icon:'🚀'}],technologies:[{name:'CUDA'},{name:'OpenCV'},{name:'C++'}],features:['Shared memory tiling','Warp divergence reduction','Histogram equalization'],architecture:{title:'Kernel Stages',description:'Pipeline of CUDA kernels',components:['Load & Normalize','Edge Enhance','Shadow Classify','Mask Refine']},github:'https://github.com/saiprudvi0102/PCNN-Shadow-Removal',demo:null},'iot-fpga':{title:'🔌 IoT Stream Analytics on FPGA',subtitle:'Low-Latency Edge Processing',image:'https://images.unsplash.com/photo-1518770660439-4636190af475?auto=format&fit=crop&w=1200&q=80',description:'Verilog + high-level synthesis for streaming analytics.',longDescription:'FPGA-based real-time anomaly detection in IoT data streams.',metrics:[{label:'Latency',value:'<1ms',icon:'⏱️'}],technologies:[{name:'Verilog'},{name:'Vitis HLS'},{name:'MQTT'}],features:['AXI-stream pipeline','Fixed-point DSP blocks','Adaptive thresholding'],architecture:{title:'FPGA Blocks',description:'Stream-centric composition',components:['Ingress Parser','Preprocess DSP','Feature Extractor','Anomaly Core','Egress Formatter']},github:'https://github.com/saiprudvi0102/AnomalyDetection',demo:null},'performance-analytics':{title:'📊 Real-time Performance Analytics',subtitle:'Big Data / Streaming / Visualization',image:'https://images.unsplash.com/photo-1551288049-bebda4e38f71?auto=format&fit=crop&w=1200&q=80',description:'High-throughput analytics platform for performance monitoring.',longDescription:'Enterprise-grade real-time analytics platform designed for monitoring application performance, processing streaming data, and providing actionable insights through interactive dashboards.',metrics:[{label:'Data Points',value:'10M/sec',icon:'📈'},{label:'Query Speed',value:'<100ms',icon:'⚡'},{label:'Storage',value:'50TB+',icon:'💾'}],technologies:[{name:'Apache Kafka'},{name:'Apache Spark'},{name:'Elasticsearch'},{name:'Kibana'},{name:'React'},{name:'Node.js'},{name:'Redis'}],features:['Real-time streaming','Interactive dashboards','Anomaly detection','Alert management','Historical analysis','Data export'],architecture:{title:'Data Pipeline',description:'Streaming analytics with visualization',components:['Data Ingestion','Stream Processing','Search Engine','Visualization Layer','Alert System','Data Storage']},github:'https://github.com/saiprudvi0102/Performance-Analytics',demo:null},'cloud-infrastructure':{title:'☁️ Cloud Infrastructure Automation',subtitle:'DevOps / IaC / Monitoring',image:'https://images.unsplash.com/photo-1451187580459-43490279c0fa?auto=format&fit=crop&w=1200&q=80',description:'Automated cloud infrastructure deployment and management.',longDescription:'Comprehensive Infrastructure as Code solution for automated cloud deployment, monitoring, and scaling across multiple cloud providers with integrated CI/CD pipelines.',metrics:[{label:'Deployment Time',value:'-80%',icon:'⚡'},{label:'Infrastructure Cost',value:'-35%',icon:'💰'},{label:'Reliability',value:'99.95%',icon:'🛡️'}],technologies:[{name:'Terraform'},{name:'Ansible'},{name:'Docker'},{name:'Kubernetes'},{name:'AWS'},{name:'Jenkins'},{name:'Prometheus'},{name:'Grafana'}],features:['Multi-cloud deployment','Auto-scaling','Infrastructure monitoring','Disaster recovery','Cost optimization','Security scanning'],architecture:{title:'Infrastructure Pipeline',description:'End-to-end cloud automation',components:['Infrastructure as Code','CI/CD Pipeline','Container Orchestration','Monitoring Stack','Security Layer','Cost Management']},github:'https://github.com/saiprudvi0102/Cloud-Infrastructure',demo:null}};}renderProject(p){return`<article class="project-detail" style="max-width:1100px;margin:0 auto;">${p.image?`<div style=\"position:relative;aspect-ratio:16/9;overflow:hidden;border-radius:16px;margin:-1rem -1rem 1.25rem;background:#111;\"><img src="${p.image}" alt="${p.title}" style="width:100%;height:100%;object-fit:cover;"/></div>`:''}<h1 style="margin:.25rem 0 .35rem;font-size:clamp(1.6rem,3.2vw,2.4rem);line-height:1.1;">${p.title}</h1><p style="margin:0 0 1rem;color:var(--text-secondary);font-size:1rem;">${p.subtitle||''}</p><p style="margin:0 0 1.25rem;font-size:.95rem;line-height:1.5;">${p.longDescription||p.description||''}</p>${p.metrics?.length?`<section style=\"margin:0 0 1.75rem;\"><h2 style=\"font-size:1rem;letter-spacing:.06em;text-transform:uppercase;font-weight:600;margin:0 0 .75rem;opacity:.9;\">Key Metrics</h2><div style=\"display:grid;gap:.75rem;grid-template-columns:repeat(auto-fill,minmax(160px,1fr));\">${p.metrics.map(m=>`<div style=\"padding:.85rem .9rem;background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.09);border-radius:12px;font-size:.75rem;letter-spacing:.05em;display:flex;flex-direction:column;gap:.35rem;\"><span style=\"font-size:1.1rem;\">${m.icon||''}</span><strong style=\"font-size:.9rem;\">${m.value}</strong><span style=\"opacity:.75;\">${m.label}</span></div>`).join('')}</div></section>`:''}${p.technologies?.length?`<section style=\"margin:0 0 1.75rem;\"><h2 style=\"font-size:1rem;letter-spacing:.06em;text-transform:uppercase;font-weight:600;margin:0 0 .75rem;opacity:.9;\">Technologies</h2><div style=\"display:flex;flex-wrap:wrap;gap:.5rem;\">${p.technologies.map(t=>`<span style=\"padding:.45rem .7rem;background:rgba(229,9,20,.15);color:var(--netflix-red,#e50914);font-size:.65rem;font-weight:500;letter-spacing:.05em;border:1px solid rgba(229,9,20,.35);border-radius:24px;\">${t.name}</span>`).join('')}</div></section>`:''}${p.features?.length?`<section style=\"margin:0 0 2rem;\"><h2 style=\"font-size:1rem;letter-spacing:.06em;text-transform:uppercase;font-weight:600;margin:0 0 .75rem;opacity:.9;\">Features</h2><ul style=\"padding:0;margin:0;list-style:none;display:grid;gap:.6rem;\">${p.features.map(f=>`<li style=\"display:flex;align-items:flex-start;gap:.55rem;font-size:.85rem;line-height:1.4;\"><span style=\"color:#4ade80;font-size:.9rem;position:relative;top:.15rem;\">✔</span><span>${f}</span></li>`).join('')}</ul></section>`:''}${(p.architecture?.components||[]).length?`<section style=\"margin:0 0 1rem;\"><h2 style=\"font-size:1rem;letter-spacing:.06em;text-transform:uppercase;font-weight:600;margin:0 0 .75rem;opacity:.9;\">${p.architecture.title}</h2><p style=\"margin:0 0 .85rem;font-size:.85rem;line-height:1.5;opacity:.9;\">${p.architecture.description||''}</p><ol style=\"margin:0;padding-left:1rem;font-size:.8rem;line-height:1.45;display:grid;gap:.4rem;\">${p.architecture.components.map(c=>`<li>${c}</li>`).join('')}</ol></section>`:''}${(p.github||p.demo)?`<footer style=\"margin:2rem 0 0;display:flex;gap:.9rem;flex-wrap:wrap;\">${p.github?`<a href=\"${p.github}\" target=\"_blank\" rel=\"noopener\" style=\"text-decoration:none;background:rgba(255,255,255,.09);border:1px solid rgba(255,255,255,.15);padding:.65rem .95rem;font-size:.7rem;letter-spacing:.06em;border-radius:8px;display:inline-flex;gap:.4rem;align-items:center;\">GitHub ↗</a>`:''}${p.demo?`<a href=\"${p.demo}\" target=\"_blank\" rel=\"noopener\" style=\"text-decoration:none;background:rgba(255,255,255,.09);border:1px solid rgba(255,255,255,.15);padding:.65rem .95rem;font-size:.7rem;letter-spacing:.06em;border-radius:8px;display:inline-flex;gap:.4rem;align-items:center;\">Live Demo ↗</a>`:''}</footer>`:''}</article>`;}}
  ...
  window.addEventListener('DOMContentLoaded',()=>{document.body.classList.add('loaded');initializeHeaderEffects();initializeCarousels();initializeNetflixEnhancements();window.__projectDetailManager=new ProjectDetailManager();new NetflixHoverManager();initializeAccessibility();
